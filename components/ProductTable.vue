<template>
    <a-table :columns="columns" :data-source="props.products" :loading="productStore.isLoading()" :scroll="{
    x: 200,
        ...
        <template #bodyCell="{ column, record }">
            ...
            <template v-else-if="column.key === 'image'">
                <a-image :src="getFullImagePath(record.image)" height="35px" class="tw-object-contain" />
            </template>
            ...
            <template v-else-if="column.key === 'action'">
                <a-row class="tw-text-center">
                    <a-button class="tw-bg-[#ffc43d] tw-text-white tw-rounded-lg tw-mr-2"
                        @click="handleClickEdit(record.id)">
                        <span>
                            <EditOutlined />
                        </span>
                    </a-button>
                    <a-button type="primary" danger class="tw-rounded-lg" @click="handleClickDelete(record.id)">
                        <span>
                            <DeleteOutlined />
                        </span>
                    </a-button>
